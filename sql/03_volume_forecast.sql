...
CREATE OR REPLACE TABLE `your-project.support_demo.volume_forecast` AS
SELECT
  forecast_timestamp,
  forecast_value,
  standard_error,
  ...
  CASE 
    WHEN forecast_value > (SELECT AVG(y) * 1.2 FROM `your-project.support_demo.daily_volumes`) 
    THEN 'Scale Up Support Team'
    WHEN forecast_value < (SELECT AVG(y) * 0.8 FROM `your-project.support_demo.daily_volumes`) 
...
FROM
  ML.FORECAST(
    MODEL (
      ...
      FROM `your-project.support_demo.daily_volumes`
      ORDER BY ds
    ),
    STRUCT(
...
forecast_summary AS (
  ...
  FROM `your-project.support_demo.volume_forecast`
)
SELECT
