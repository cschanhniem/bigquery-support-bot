...
CREATE OR REPLACE TABLE `${PROJECT_ID}.support_demo.evaluation_baseline` AS
WITH baseline_heuristics AS (
  ...
  FROM `${PROJECT_ID}.support_demo.raw_tickets`
...
LEFT JOIN `${PROJECT_ID}.support_demo.daily_insights` di
...
CREATE OR REPLACE TABLE `${PROJECT_ID}.support_demo.model_performance` AS
WITH confusion_matrix AS (
  SELECT
    ai_sentiment,
    baseline_sentiment,
    COUNT(*) as count
  FROM `${PROJECT_ID}.support_demo.evaluation_baseline`
...
forecast_accuracy AS (
  ...
  FROM `${PROJECT_ID}.support_demo.ticket_forecast` tf
  INNER JOIN (
...
CREATE OR REPLACE TABLE `${PROJECT_ID}.support_demo.root_cause_trends` AS
SELECT
  root_cause,
  COUNT(*) as total_occurrences,
  COUNT(*) / (SELECT COUNT(*) FROM `${PROJECT_ID}.support_demo.daily_insights`) * 100 as percentage,
...
FROM `${PROJECT_ID}.support_demo.daily_insights`
GROUP BY root_cause
ORDER BY total_occurrences DESC;

-- Data quality assessment
CREATE OR REPLACE TABLE `${PROJECT_ID}.support_demo.data_quality_report` AS
...
CREATE OR REPLACE VIEW `${PROJECT_ID}.support_demo.executive_summary` AS
SELECT
  -- Performance Summary
  ...
  -- Current State
  (SELECT COUNT(*) FROM `${PROJECT_ID}.support_demo.daily_insights`) as days_analyzed,
  (SELECT root_cause FROM `${PROJECT_ID}.support_demo.root_cause_trends` 
   ORDER BY total_occurrences DESC LIMIT 1) as top_root_cause,
  (SELECT ROUND(negative_sentiment_rate, 1) 
   FROM `${PROJECT_ID}.support_demo.root_cause_trends`
   ORDER BY total_occurrences DESC LIMIT 1) as top_cause_negative_rate,
   
  -- Data Quality
  (SELECT AVG(CAST(completeness_pct AS FLOAT64)) 
   FROM `${PROJECT_ID}.support_demo.data_quality_report`) as avg_data_quality,
...
FROM `${PROJECT_ID}.support_demo.model_performance` mp;
...
SELECT * FROM `${PROJECT_ID}.support_demo.executive_summary`;
